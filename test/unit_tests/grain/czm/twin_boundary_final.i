# Twin Boundary Test: ±31.9° Configuration
# Symmetric twin with ~64° total misorientation
#
# Expected behavior:
# - Elastic: Similar (but not identical) response due to symmetry
# - Yield: Nearly simultaneous (challenging for convergence!)
# - Requires stabilization: higher viscosity, delayed softening
...
[Materials]
  ...
  [plasticity_grain1]
    ...
    # TWIN STABILIZATION: Delayed and gentler softening
    postyield_mode = 'exp_softening'
    residual_strength = 0.7
    kmax = 0.002              # Delayed (2× standard)
    kmin = 0.025              # Longer range
    kslope = 50.0             # Gentler softening
    
    # TWIN STABILIZATION: Higher viscosity
    viscosity_mode = 'linear'
    eta = 0.005               # 2.5× standard for regularization
    m = 0.001
  ...
  #========================================
  # GRAIN 2: -31.9° twin orientation (mirror)
  #========================================
  [elasticity_grain2]
    type = ComputeElasticityTensor
    fill_method = symmetric9
  ...
  [plasticity_grain2]
    ...
    # TWIN STABILIZATION: Same as grain 1
    postyield_mode = 'exp_softening'
    residual_strength = 0.7
    kmax = 0.002
    kmin = 0.025
...
[Executioner]
  type = Transient
  dt = 0.05                # Smaller timestep for twin stability
  end_time = 150
  solve_type = 'NEWTON'
  ...
  [TimeStepper]
    ...
    optimal_iterations = 8  # Tighter control
    iteration_window = 2
    growth_factor = 1.1     # Slower growth
    cutback_factor = 0.4    # More aggressive cutback
  []
[]
