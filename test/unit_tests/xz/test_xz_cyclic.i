# Unit Test: XZ-Shear
# Expected: Yield at 5080 MPa, soften to 3556 MPa
...
[BCs]
  # ============================================
  # XZ SHEAR BCs: Fix back (z=0), shear front (z=1) in x
  # ============================================
  ...
  # Front face (z=1): apply shear in x, fix z
  [shear_front_x]
    type = FunctionDirichletBC
    variable = disp_x
    boundary = 'front'
    # γ_xz = u_x / h = 0.001*t, so σ_xz = G*γ = 31100*0.001*t = 31.1*t MPa
    # Yield at σ_xz = 5080: t = 5080/31.1 ≈ 163.3 s
    function = 'if(t<=240, 0.001*t, 0.001*(480-t))'
  []
  [fix_front_z]
    type = DirichletBC
    variable = disp_z
    ...
    value = 0
  []
  # Note: disp_y on front is FREE
  
  # Top/bottom faces: constrain y for plane strain
  [fix_top_y]
    type = DirichletBC
    variable = disp_y
    boundary = 'top'
...
[Outputs]
  csv = true
  file_base = xz_shear_out_cyclic_perfect
