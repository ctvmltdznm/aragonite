# Two-Grain Aragonite CZM Test - Mixed mode test
...
[AuxKernels]
  ...
  [mode_ratio_aux]
    type = ParsedAux
    variable = mode_ratio
    coupled_variables = 'normal_jump tangent_jump'
    expression = 'if(abs(normal_jump)>1e-20, abs(tangent_jump/normal_jump), 1000)'
    boundary = 'grain_1_grain_2'
    execute_on = 'TIMESTEP_END'
  []
[]
...
[BCs]
  ...
  [fix_left_z]
    ...
    value = 0
  []
  # RIGHT face (x=2.0, grain 2 boundary): SHEAR in Y direction
  [shear_right_y]
  ...
  [fix_right_z]
    ...
    variable = disp_z
    boundary = 'right'
    value = 0                    # Prevent out-of-plane
  []
  
  # BACK face (z=0): Fix Z to prevent rigid body motion
  [fix_back_z]
    type = DirichletBC
    variable = disp_z
    boundary = 'back'
...
[Postprocessors]
  ...
  []
  # Interface quantities (averaged over interface)
  [delta_n]
